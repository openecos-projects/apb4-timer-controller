...
def main() -> None:
    ...
    if data["uid"] != "ip-000006":
        raise ValueError("uid must be ip-000006")
    if data["repo_name"] != "ip-000006":
        raise ValueError("repo_name must be ip-000006")
    if data["category"] != "peripheral":
        raise ValueError("category must be peripheral")
    if data["links"]["repository"] != "https://github.com/oscc-ip/timer":
        raise ValueError("upstream repository link must reference oscc-ip/timer")
